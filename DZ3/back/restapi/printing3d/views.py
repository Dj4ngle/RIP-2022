...
class SellViewSet(viewsets.ModelViewSet):
    queryset = Sell3d.objects.all().order_by('-sell_date')
...
def update_model(request):
    ...
    if ssid is not None:
        ...
        if user.is_staff == 1:
            Model3d.objects.filter(id=id).update(name=name, description=description, price=price)
...
def create_model(request):
    ...
    if ssid is not None:
        user = User.objects.get(username=session_storage.get(ssid).decode())
        if user.is_staff == 1:
            Model3d.objects.create(name=name, description=description, price=price, image_path=image_path)
            response = Response("{\"status\": \"ok\"}", content_type="json")
        else:
            response = Response("{\"status\": \"access denied\"}", content_type="json")
    else:
...
def create_sell(request):
    ...
    if user is not None:
        if user.is_staff:
            u = Sell3d.objects.create(status=status, comment=comment, id_user=user.id)
...
def update_sell(request):
    ...
    status = data["status"]
    if ssid is not None:
        Sell3d.objects.filter(id=id).update(status=status, change_date=timezone.now())
        response = Response("{\"status\": \"ok\"}", content_type="json")
    else:
        response = Response("{\"status\": \"you have to logIn\"}", content_type="json")
    return response
...
@api_view(["PUT"])
def update_sell_comment(request):
    data = json.loads(request.body)
    ssid = request.COOKIES.get("session_cookie")
    id = data["id"]
    status = data["status"]
    comment = data["comment"]
    if ssid is not None:
        Sell3d.objects.filter(id=id).update(status=status, change_date=timezone.now(), comment=comment)
        response = Response("{\"status\": \"ok\"}", content_type="json")
    else:
        response = Response("{\"status\": \"you have to logIn\"}", content_type="json")
    return response
...
def create_purchase(request):
    ...
    user = User.objects.get(username=session_storage.get(ssid).decode())
    if user is not None:
        Purchase.objects.create(id_purchase=id_purchase, id_model_id=id_model, quantity=quantity, colour=colour, size=size)
        return HttpResponse("{\"status\": \"ok\"}", content_type='json')
    else:
        return HttpResponse("{\"status\": \"error\", \"error\": \"user creation failed\"}", content_type='json')
...
def update_purchase(request):
    ...
    size = data["size"]
    if ssid is not None:
        Purchase.objects.filter(id=id).update(quantity=quantity, colour=colour, size=size)
        response = Response("{\"status\": \"ok\"}", content_type="json")
    else:
        response = Response("{\"status\": \"you have to logIn\"}", content_type="json")
    return response
